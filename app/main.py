...
# Base multi-release overrides load first; runtime handoff, legacy compatibility
# and reschedule cleanup extend them next. Schema migration runs after DB init.
from app.jackside_multi_issue import install_jackside_multi_issue
...
from app.member_identity_qr import install_member_identity_qr
...
def _install_extensions(application):
    ...
    application = install_member_identity_qr(application)
    ...
    application = install_telegram_safety_hotfix(application)
    ...
    application = install_jackside_final_outcome_only(application)
