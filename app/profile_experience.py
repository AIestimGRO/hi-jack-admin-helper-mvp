...
def title_collection_payload(conn, *, client_id: int) -> dict[str, Any]:
    ...
    for row in conn.execute(
        ...
        if int(row["id"]) in active_title_ids:
            continue
        item = _as_title(row, state="locked", temporary=str(row["title_type"]) == "temporary")
        locked_titles.append(item)

    locked_achievements = []
    for row in conn.execute(
        "SELECT id,name,description,icon,icon_path FROM achievement_definitions WHERE is_enabled=1 ORDER BY id",
    ).fetchall():
        if int(row["id"]) in active_achievement_ids:
            continue
        locked_achievements.append(_as_achievement(row, state="locked"))

    # DOM order matters: CSS fills a two-row horizontal grid by columns, so the
    # active collection occupies the first visible cells in both rows.
