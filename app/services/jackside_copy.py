    ...
    for item in RESULT_COPY_BY_RANGE:
        if item["min_correct"] <= score <= item["max_correct"]:
            ...
            if qualified:
                ...
                message = (
                    f"{message} Все ответы правильные, но результат не прошёл "
                    "в финал по условиям общего дедлайна 4:14."
                )
            else:
                message = (
                    f"{message} В этот раз вы не проходите в финальный стол: "
                    "в финал проходят только участники, которые правильно ответили "
                    "на все вопросы основной части до общего дедлайна 4:14. "
                    "Спасибо за участие! Не расстраивайтесь — приходите на "
                    "следующий квиз, будем ждать."
                )
