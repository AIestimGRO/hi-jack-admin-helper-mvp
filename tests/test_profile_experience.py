...
def test_title_collection_lists_active_first_and_all_remaining_locked(tmp_path: Path) -> None:
    ...
    with transaction(db_path) as conn:
        ...
        conn.execute(
            """
            INSERT INTO member_titles(client_id,title_definition_id,selected)
            VALUES (?,?,1)
            """,
    ...
    assert payload["items"][0]["kind"] == "title"
    ...
    with transaction(db_path) as conn:
        ...
        conn.execute(
            ...
            INSERT INTO member_titles(
                client_id,title_definition_id,temporary_period_id,expires_at
            ) VALUES (?,?,?,?)
            """,
            (
                client_id,
