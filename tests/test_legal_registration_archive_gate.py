...
def _master_login(client: TestClient) -> None:
    ...
    response = client.post(
        "/login",
        data={"username": "master", "pin": "2468", "csrf_token": _csrf(page.text)},
...
def test_supplied_legal_documents_and_registration_labels_are_kept_separate() -> None:
    ...
    agreement = (root / "data/legal/01_user_agreement.txt").read_text(encoding="utf-8")
    policy = (root / "data/legal/02_privacy_policy.txt").read_text(encoding="utf-8")
    consent = (root / "data/legal/03_personal_data_consent.txt").read_text(encoding="utf-8")
    marketing = (root / "data/legal/04_marketing_consent.txt").read_text(encoding="utf-8")
    image = (root / "data/legal/05_image_consent.txt").read_text(encoding="utf-8")
    rating = (root / "data/legal/06_public_rating_consent.txt").read_text(encoding="utf-8")
    ...
    assert "Я принимаю Пользовательское соглашение и Правила Hi, Jack! Club" in register
    ...
    assert 'name="marketing_consent" type="checkbox" value="true" data-registration-marketing>' in register


def test_schema_replaces_legacy_seed_but_preserves_master_published_version(tmp_path: Path) -> None:
    db_path = tmp_path / "legal.sqlite3"
    init_db(db_path)
    with transaction(db_path) as conn:
        legacy = conn.execute(
            "SELECT version FROM legal_documents WHERE code='privacy' AND is_active=1"
        ).fetchone()
        assert legacy["version"] == "1.0"

        ensure_legal_registration_schema(conn)
        current = conn.execute(
            "SELECT version,title FROM legal_documents WHERE code='privacy' AND is_active=1"
        ).fetchone()
        assert current["version"] == LEGAL_VERSION
        assert current["title"] == "Пользовательское соглашение и Правила Hi, Jack! Club"
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM legal_reference_documents WHERE is_active=1"
            ).fetchone()[0]
            == 4
        )
        ...
        conn.execute(
            ...
            VALUES ('privacy','2.0','Master edition','Custom legal text that is intentionally kept by seeding.',1)
            """
        )
        ensure_legal_registration_schema(conn)
        preserved = conn.execute(
            "SELECT version,title,content FROM legal_documents WHERE code='privacy' AND is_active=1"
        ).fetchone()
...
def test_optional_and_public_rating_consent_proof_is_versioned_and_redacted_on_delete(
    ...
    with transaction(db_path) as conn:
        ...
        client_id = int(
            conn.execute(
                "INSERT INTO clients(first_name,client_status,source) VALUES ('Legal User','existing','test')"
            ).lastrowid
        )
        account_id = int(
            conn.execute(
        ...
        state = conn.execute(
            "SELECT document_version,granted FROM member_optional_consent_state WHERE account_id=? AND code='marketing-consent'",
            (account_id,),
        ).fetchone()
        ...
        rating = conn.execute(
            "SELECT document_version,categories_json,granted FROM member_public_rating_consent_state WHERE account_id=?",
            (account_id,),
        ).fetchone()
        ...
        optional_event = conn.execute(
            "SELECT ip_hash,user_agent FROM member_optional_consent_events WHERE account_id=? ORDER BY id DESC LIMIT 1",
            (account_id,),
        ).fetchone()
        rating_event = conn.execute(
            "SELECT ip_hash,user_agent FROM member_public_rating_consent_events WHERE account_id=? ORDER BY id DESC LIMIT 1",
            (account_id,),
        ).fetchone()
...
def test_registration_flow_has_two_mandatory_actions_policy_link_and_optional_marketing(
    ...
    with client:
        ...
        assert "Я принимаю Пользовательское соглашение и Правила Hi, Jack! Club" in first.text
        ...
        assert "Хочу получать новости, акции и специальные предложения Hi, Jack! Club" in profile.text
...
def test_registration_rejects_under_18_before_account_creation(tmp_path: Path) -> None:
    ...
    with client:
        ...
        today = date.today()
        underage = date(today.year - 17, today.month, min(today.day, 28)).isoformat()
        response = client.post(
...
def test_quiz_requires_member_account_and_preserves_target_link(tmp_path: Path) -> None:
    ...
    with client:
        ...
        assert "/account/register?next=" in login_page.text
...
def test_master_registry_separates_active_and_archive_users(tmp_path: Path) -> None:
    ...
    with client:
        ...
        with transaction(settings.db_path) as conn:
            active_id = int(
                conn.execute(
                    "INSERT INTO clients(first_name,client_status,source) VALUES ('Active Legal','existing','test')"
                ).lastrowid
            )
            conn.execute(
                """
            ...
            conn.execute(
                "INSERT INTO clients(first_name,client_status,source) VALUES ('No Account Archive','existing','test')"
            )
            deleted_id = int(
                conn.execute(
                    "INSERT INTO clients(first_name,client_status,source) VALUES ('Deleted Archive','deleted','test')"
                ).lastrowid
            )
            conn.execute(
                """
                INSERT INTO member_accounts(client_id,email,email_normalized,password_hash,is_active,email_verified_at)
                VALUES (?,?,?,?,0,CURRENT_TIMESTAMP)
                """,
                (deleted_id, "deleted-test@deleted.invalid", "deleted-test@deleted.invalid", "deleted"),
            )
        ...
        legal = client.get("/master/legal-documents")
        assert legal.status_code == 200
        assert "Юридические документы" in legal.text
        assert "Пользовательское соглашение и Правила Hi, Jack! Club" in legal.text
        assert "Согласие на получение рекламных и информационных сообщений" in legal.text
